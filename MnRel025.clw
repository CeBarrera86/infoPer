   ...
   INCLUDE('abrppsel.inc'),ONCE
                     ...
                     MAP
                       INCLUDE('MNREL025.INC'),ONCE        !Local module procedure declarations
                     END
...
!!! </summary>
RelojArmarPartesPDF PROCEDURE (fecha,Sector,sede,ncopias)
...
Detail                 DETAIL,AT(,,,5),USE(?Detail),FONT('Arial',11)
                         STRING('Sede'),AT(91,0),USE(?String5),FONT(,,,FONT:regular),TRN
                         STRING('Sector'),AT(6,0),USE(?String6),FONT(,,,FONT:regular),TRN
                         STRING(@s22),AT(110,0,46,6),USE(SED:SED_Descripcion),FONT(,,,FONT:regular),TRN
                         STRING(@d06),AT(170,0),USE(fecha),FONT(,,,FONT:bold,CHARSET:ANSI),TRN
                         STRING('Fecha:'),AT(157,0),USE(?String9),FONT(,,,FONT:regular),TRN
                         STRING(@s30),AT(27,0,64,6),USE(LUG:LUG_Descripcion),FONT(,,,FONT:bold),TRN
...
ReturnValue          BYTE,AUTO
  ...
  CODE
  GlobalErrors.SetProcedureName('RelojArmarPartesPDF')
  ...
  SELF.Open(ProgressWindow)                                ! Open window
      loc:sector = sector
      loc:sede = sede
      loc:nombre_pdf = 'c:\pdf\parte' & format(loc:sector,@n03) & '.pdf'
...
PDFReporter.SetUp PROCEDURE
  ...
  CODE
  PARENT.SetUp
  SELF.SetFileName(loc:nombre_pdf)
  SELF.SetDocumentInfo('Parte Diario Personal','MnRel','MnRel','Oficina Personal','Oficina Personal','')
