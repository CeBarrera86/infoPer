   ...
   INCLUDE('ABREPORT.INC'),ONCE
                     ...
                     MAP
                       INCLUDE('INFOPER019.INC'),ONCE        !Local module procedure declarations
                     END
...
!!! </summary>
ArmarDatosLiquidacion PROCEDURE 
...
ReturnValue          BYTE,AUTO
  ...
  CODE
  GlobalErrors.SetProcedureName('ArmarDatosLiquidacion')
  ...
  CREATE(DESFAC)
  if errorcode() then
     message('no se pudo inicializar datos para LIQUIDACION')
  end
  Relate:DESFAC.Open                                       ! File DESFAC used by this procedure, so make sure it's RelationManager is open
...
ReturnValue          BYTE,AUTO
  ...
  CODE
  DES:PERIODO=FORMAT(ECP:ANIO,@N04) & FORMAT(ECP:PERIODO,@N02)
  DES:LEGAJO=Ecp:Legajo
  DES:SERVICIO=Ecp:codempresa
  DES:IMPORTE=Ecp:Importe
  ADD(DESFAC)
  IF ERRORCODE() THEN
     GET(DESFAC,DES:KEY_DESFAC)
     DES:IMPORTE+=Ecp:Importe
     PUT(DESFAC)
  END
